FROM registry.access.redhat.com/ubi9/go-toolset:1.22.7 as osdk-builder
...
FROM registry.access.redhat.com/ubi9/go-toolset:1.22.7 as opm-builder

COPY --chown=default ./operator-registry/. /opt/app-root/src/
WORKDIR /opt/app-root/src
RUN ls -l && CGO_ENABLED=0 GOOS=linux go build -a -o opm cmd/opm/main.go

FROM registry.access.redhat.com/ubi9/go-toolset:1.22.7 as kustomize-builder
